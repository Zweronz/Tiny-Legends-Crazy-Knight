...
public class Crazy_PlayerControlMage_Net : Crazy_PlayerControl_Net
{
	[CompilerGenerated]
	private sealed class _003CMeteorAttackHurt_003Ec__AnonStorey15
	{
		internal GameObject curEnemyObj;

		internal bool _003C_003Em__8(GameObject a)
		{
	...
	private string m_attackName = "Attack01";
	...
	protected override void Roll()
	{
		...
		blinkStartEffectobj.transform.position = base.transform.position;
		blinkStartEffectobj.transform.GetComponentInChildren<ParticleSystem>().Play(true);
		PlayTAudio("Shanxian");
		StartCoroutine(Blink(rolldir, 6f));
	}
	...
	private IEnumerator Blink(Vector3 movedir, float movedistance)
	{
		...
		blinkEndEffectobj.transform.GetComponentInChildren<ParticleSystem>().Play(true);
	}
	...
	{
		if (m_attackName == "Attack01")
		{
			m_attackName = "Attack01_Staff01";
		}
		else
		{
			m_attackName = "Attack01";
		}
		usingskill = false;
		if (is_attack_pause)
		{
	...
	private void MeteorAttackHurt(int index)
	{
		...
		{
			_003CMeteorAttackHurt_003Ec__AnonStorey15 _003CMeteorAttackHurt_003Ec__AnonStorey = new _003CMeteorAttackHurt_003Ec__AnonStorey15();
			if (!Crazy_GlobalData.enemyList.TryGetValue(item, out _003CMeteorAttackHurt_003Ec__AnonStorey.curEnemyObj) || attacklist.Exists(_003CMeteorAttackHurt_003Ec__AnonStorey._003C_003Em__8))
			{
				continue;
			}
	...
	public override string GetAttackStatusName()
	{
		return m_attackName;
	}
}
