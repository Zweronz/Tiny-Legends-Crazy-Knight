...
public class Crazy_EnemyControl_Remote : Crazy_EnemyControl
{
	...
	protected override void updateData(float deltatime)
	{
		...
		if (flag && !flag2)
		{
			...
			{
				Vector2 vector = Crazy_Global.Rotate(new Vector2(0f - curMoveDir.x, 0f - curMoveDir.z), Random.Range(-30, 30));
				curMoveDir = new Vector3(vector.x, 0f, vector.y);
			}
		}
		else if (flag2 && !flag3)
		{
			Vector2 vector2 = Crazy_Global.Rotate(new Vector2(0f - curMoveDir.x, 0f - curMoveDir.z), Random.Range(-30, 30));
	...
	protected void Remote()
	{
		...
		{
			GameObject gameObject2 = Object.Instantiate(remote_item, gameObject.transform.position, base.transform.rotation) as GameObject;
			...
			Vector3 vector = ((i != 0) ? new Vector3(Random.Range(-7f, 7f), 0f, Random.Range(-7f, 7f)) : Vector3.zero);
	...
	protected override void Die()
	{
		...
		Crazy_Statistics.AddMonsterKillNumber(Crazy_MonsterType.Ranged, m_id);
		Crazy_TaskManager.GetInstance().updateTask(Crazy_TaskId.Task06, m_type - 1, 0f);
		Crazy_EnemyManagement.RemoveRangedEnemyNumber();
	}
