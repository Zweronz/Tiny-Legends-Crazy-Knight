...
public class Crazy_EnemyControl_Boss_dragon_Net : Crazy_EnemyControl_Boss_Net
{
	...
	{
		preStart();
		ReadStatus("dragon_netskill", "dragon_netstatus");
		base.GetComponent<Animation>().wrapMode = WrapMode.Once;
		base.GetComponent<Animation>()["standby_merge"].wrapMode = WrapMode.Loop;
		base.GetComponent<Animation>()["attack_00_merge"].wrapMode = WrapMode.ClampForever;
		base.GetComponent<Animation>()["attack_01_merge"].wrapMode = WrapMode.ClampForever;
		base.GetComponent<Animation>()["run_merge"].wrapMode = WrapMode.Loop;
	...
	{
		EventAnimation(base.GetComponent<Animation>()["attack_00_merge"].clip, 39, "OffAttack", 0, 0f, string.Empty);
		EventAnimation(base.GetComponent<Animation>()["attack_01_merge"].clip, 48, "OffAttack", 0, 0f, string.Empty);
	}
	...
	protected override void PlayAnimation(Crazy_MonsterStatus toStatus)
	{
		...
		case Crazy_MonsterStatus.Move:
			base.GetComponent<Animation>()["run_merge"].speed = speedWalk * (1f - skillmoverate) * moverate / 5f;
			base.GetComponent<Animation>().CrossFade("run_merge", 0.01f);
			break;
		case Crazy_MonsterStatus.Run:
			base.GetComponent<Animation>()["run_merge"].speed = speedRun * (1f - skillmoverate) * moverate / 5f;
			base.GetComponent<Animation>().CrossFade("run_merge", 0.01f);
			break;
