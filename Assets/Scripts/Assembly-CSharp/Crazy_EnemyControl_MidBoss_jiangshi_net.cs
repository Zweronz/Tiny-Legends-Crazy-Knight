...
public class Crazy_EnemyControl_MidBoss_jiangshi_net : Crazy_EnemyControl_MidBoss_Net
{
	private void Start()
	{
		preStart();
	...
	protected override void PlayAnimation(Crazy_MonsterStatus toStatus)
	{
		...
		case Crazy_MonsterStatus.Move:
			base.GetComponent<Animation>()["forward01_merge"].speed = speedWalk * (1f - skillmoverate) / 1.5f;
			base.GetComponent<Animation>().CrossFade("forward01_merge", 0.1f);
			break;
		case Crazy_MonsterStatus.Run:
			base.GetComponent<Animation>()["forward01_merge"].speed = speedRun * (1f - skillmoverate) / 1.5f;
			base.GetComponent<Animation>().CrossFade("forward01_merge", 0.1f);
			break;
	...
	{
		return new Renderer[1] { base.gameObject.transform.Find("jiangshi2").gameObject.GetComponent<Renderer>() };
	}

	protected override GameObject[] FindHeadNode()
	{
		return new GameObject[1] { base.gameObject.transform.Find("Root/Head/Head01").gameObject };
	}
}
